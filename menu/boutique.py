    ...
    def draw(self):
        ...
        if items_ids and self.selection < len(items_ids):
            ...
            if item_data:
                ...
                text_render_centered_left(self.jeu.ui_surface, item_data["nom"], "bold", color=(255, 200, 100), pos=(detail_x, detail_y), size=32)
                text_render_centered_left(self.jeu.ui_surface, f"Type: {item_data.get('type', 'Inconnu')}", "imitalic", color=(200, 200, 200), pos=(detail_x, detail_y + 40), size=20)

                desc = item_data.get("description", "")
                text_render_centered_left(self.jeu.ui_surface, desc, "regular", color=(255, 255, 255), pos=(detail_x, detail_y + 80), size=22)

                text_render_centered_left(self.jeu.ui_surface, "[ESPACE] Utiliser / [ECHAP] Retour", "regular", color=(150, 150, 150), pos=(detail_x, 600), size=18)

        elif not items_ids:

            text_render_centered(self.jeu.ui_surface, "L'inventaire est vide", "imitalic", color=(150, 150, 150), pos=(self.jeu.WIDTH // 2, self.jeu.HEIGHT // 2), size=30)