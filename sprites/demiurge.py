        ...
        if self.alive and player.alive:
            ...
            if self.angry:
                if self.anger_timer % 100 == 0:
                    self.fireballs.append(Fireball(self.rect.centerx, self.rect.centery, -1 if self.flip else 1, 15,
                                                   self.image_scale // 2,
                                                   pygame.image.load('./assets/sprites/fireball.png').convert_alpha()))
                self.anger_timer += 1
...
class Fireball:
    ...
    def collision(self, player):
        ...
        if overlap and not self.has_hit:
            if not player.blocking or (player.flip and self.direction == -1) or (
                    not player.flip and not self.direction == -1):
